      ...
      <div className="flex items-start justify-center mt-10 bg-navbarcolor">
        <div className="w-3/4 flex-col flex items-center justify-center">
          ...
          <div className="flex flex-col border border-gray-400 w-image bg-white mt-10">
            <h1 className="text-xl font-bold text-borderedgecolor m-5">
              Details
            </h1>
        ...
        <div className="w-1/2 flex flex-col items-start justify-start">
          <div className="bg-white w-96  h-auto shadow-lg border border-gray-400 p-2">
            <div className="flex items-center justify-start">
              <p className="font-bold text-xl m-5 text-borderedgecolor">
                ₹ {product?.Price}
              </p>
              <img
                src={ShareImage}
            ...
            <p className="font-light text-sm ml-5 mt-2">
              {product?.PostingDate}
            </p>
            <p className="font-light text-sm ml-5 mt-2">
              {product?.Kilometers
                ? 'Kilometeres' + '  ' + product?.Kilometers
                : ''}{' '}
            </p>
            <p className="font-light text-sm ml-5 mt-2">
          ...
          <div className="bg-white w-96 mt-10 h-auto shadow-lg border border-gray-400 p-2">
            <div className="flex items-center justify-start">
              <p className="font-bold text-xs m-5 text-borderedgecolor">
                {product?.Username}
              </p>
              <img
                src={RightArrow}
                className="h-6 w-6"
                onClick={() => profileHandler(product?.Username)}
              />
            </div>
            <button className="w-full p-2 h-max border border-borderedgecolor">
              <p
                className="font-bold text-borderedgecolor text-sm"
                onClick={() => chatHandler(product?.Username)}
              >
                Chat With Seller
              </p>
            </button>
          ...
          <div className="bg-white w-96 mt-10 h-auto shadow-lg border border-gray-400 p-2">
            <div className="flex items-center flex-col justify-between">
              <p className="font-bold text-xl mt-2 text-borderedgecolor">
                Posted In
              </p>
      ...
      {chatdialog && (
        <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50">
          <div className="bg-white h-96 w-96 p-4 flex items-center justify-start flex-col rounded shadow-lg">
            ...
            <h1 className="text-xl font-bold text-borderedgecolor mt-10">
              Send Message To {profile}
            </h1>
            <input
              type="text"
              className="w-72 border border-t-white border-l-white border-r-white border-b-black mt-10"
              placeholder="write your message here"
              value={message}
              onChange={(e) => setMessage(e.target.value)}
            />
            <button
              className="w-72 mt-10 bg-borderedgecolor h-10 font-bold text-white"
